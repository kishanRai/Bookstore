...
/**
 * Checks checkout orchestration, replay, stock reservation, ownership and failure behavior using isolated collaborators.
 */
class OrderServiceTest {
    ...
    private final StockReservationStrategy stockReservation = mock(StockReservationStrategy.class);
    private final Clock clock = Clock.fixed(Instant.parse("2026-09-13T12:00:00Z"), ZoneOffset.UTC);
    private final OrderService service = new OrderService(orders, carts, lock, clock, events, stockReservation);

    private CartItem item() {
        var book = BookTestDataBuilder.aBook().withId(1).withPrice("12.50").build();
    ...
    @Test void replayDoesNotReadOrClearANewCart() {
        ...
        verifyNoInteractions(stockReservation);
    ...
    @Test void checkoutUsesTheClockAndCoordinatesPersistenceInOrder() {
        ...
        var sequence = inOrder(lock, orders, carts, stockReservation, events);
        ...
        sequence.verify(stockReservation).reserve(1L, 2);
    ...
    @Test void insufficientStockDoesNotCreateAnOrderOrClearTheCart() {
        var item = item();
        when(carts.findCart(7L)).thenReturn(List.of(item));
        doThrow(new InsufficientStockException("Book 1", 1, 2)).when(stockReservation).reserve(1L, 2);
        assertThatThrownBy(() -> service.checkout(7, UUID.randomUUID())).isInstanceOf(InsufficientStockException.class);
        verify(orders, never()).saveAndFlush(any());
        verify(carts, never()).deleteCart(anyLong());
        verifyNoInteractions(events);
    }
